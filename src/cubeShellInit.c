...
/*-----------------------------------------------------------------------*/
{
  ...
  if (config.masCouple == 0 || config.masInitFromOuterBoundary == 0){
    initSphereCoords( INNER_SHELL );
  }
  else if (config.masInitFromOuterBoundary > 1){
...
/*-----------------------------------------------------------------------*/
{
  ...
  for (face = 0; face < NUM_FACES; face++) {
    for (row = 0; row < FACE_ROWS; row++) {
      for (col  = 0; col < FACE_COLS; col++) {
        ...
        if (config.masInitRadius > 0.0)
          rmag = config.masInitRadius / config.rScale;
        else {
          rmag = 0.99 * config.masRadialMax * RSAU / config.rScale;
          config.masInitRadius = rmag * config.rScale;
        }
  ...
  do {
      ...
      for (row = 0; row < FACE_ROWS; row++) {
        for (col = 0; col < FACE_COLS; col++) {
          ...
          if (nodeFlag[idx_frc(face,row,col)] == 0) {
            ...
            if ( rmag > (config.masRadialMin * RSAU / config.rScale) ) {
    ...
    if ( (config.masInitFromOuterBoundary == 1) && (crossCount > 0) )
        loopFlag = 1;
    if ( (config.masInitFromOuterBoundary == 2) && (crossCount == FRC) )
        loopFlag = 1;
    ...
    for (row = 0; row < FACE_ROWS; row++) {
      for (col = 0; col < FACE_COLS; col++) {
        ...
        while ( (n < N) && (diff0 > tol) ) {
          ...
          } while (cartToSphPos(rWalk).r * config.rScale < config.masInitRadius);
